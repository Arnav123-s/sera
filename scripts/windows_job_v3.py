...
class WindowsJob:
    def __init__(self, process, *, memory_bytes):
        ...
        try:
            ...
            if type(memory_bytes) is not int or memory_bytes < 256*1024**2:
                raise ValueError("A positive realistic memory cap is required")
    ...
    def sample(self):
        ...
        return {"peak_job_committed_bytes": int(value.PeakJobMemoryUsed),
                ...
                "configured_job_memory_limit_bytes": int(value.JobMemoryLimit),
                "configured_limit_flags": int(value.BasicLimitInformation.LimitFlags),
