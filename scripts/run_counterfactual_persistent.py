...
ALLOWED = {"scripts.counterfactual_inventory", "scripts.counterfactual_study", "scripts.counterfactual_use",
           ...
           "scripts.counterfactual_live", "scripts.counterfactual_audit", "pytest", "ruff"}


def write(path, value):
    temporary = path.with_suffix(".tmp")
    temporary.write_text(json.dumps(value, indent=2) + "\n", encoding="utf-8", newline="\n")
    # Windows readers and indexers may briefly hold a handle without delete
    # sharing. A metadata collision must not immediately terminate the worker.
