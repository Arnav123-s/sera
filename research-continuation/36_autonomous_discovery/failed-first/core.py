    ...
    if not word:
        ...
        if op == "M":
            ...
            if not check(domain, list(map(str, inner)), proposed)["accepted"]:
                raise ValueError("An imagined primitive needs further acquisition")
...
def signatures(owner, depth, seed):
    ...
    columns = [[v for p in imagined for v in execute(owner, w, p, cache, counts)] for w in names]
    return names, columns, {"imagined": imagined, **counts,
                           "program_evaluations": len(names)*len(imagined), "shared_prefix_cache": True}
