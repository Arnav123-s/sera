...
class Arxiv:
    ...
    def fetch(self, url, *, refresh=False):
        ...
        if record.exists() and not refresh:
            ...
            raw = (self.directory / saved["body"]).read_bytes()
        ...
        with (self.directory / "request.lock").open("x"):
            try:
    ...
    def paper(self, identifier, kind="html"):
        ...
        return self.fetch(f"https://arxiv.org/{kind}/{identifier}")
    ...
    def search(self, query, limit=3):
        ...
        return self.fetch("https://export.arxiv.org/api/query?" + urlencode(
            {"search_query": query, "start": 0, "max_results": limit, "sortBy": "relevance"}))
