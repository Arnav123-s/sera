...
Run 49 exposed a second imported assessor alias in the Stage 34 reading bridge. The adapter now covers both explicit restore paths. The preserved completed Stage 34 witness is published here, without its issuer key, and pinned by SHA-256. Its receipt must exactly equal the already-public Stage 34 qualification, and its 256 reference cases must replay. The regression test covers both imported aliases. These failures did not restart any scientific training or change evaluated results.
