...
class Runtime:
    def __init__(self, checkpoint, record=None, *, migrate=False):
        ...
        factual, library = self.session.learner.legacy.snapshot(), self.session.learner.library.record()
        ...
        if record is not None:
            permitted_source = record["source"] == source() or (migrate and record["source"] in RUNTIME_SOURCES)
            if record["schema"] != "sera.language-inquiry.1" or not permitted_source:
            ...
            self.graph = Investigation.restore(self.owner, record["graph"], migrate=migrate)
        else:
            self.graph = Investigation(self.owner)
        self.reproof = self.session.learner.rebind(factual, library)
    ...
    def observe(self, action):
        ...
        candidate = Runtime(self.checkpoint, self.snapshot())
        factual, library = candidate.session.learner.legacy.snapshot(), candidate.session.learner.library.record()
        row = candidate._observe(action)
        learner = candidate.session.learner
        candidate.reproof = learner.rebind(factual, library)
    ...
    def teach(self, phrase, action, *, checkpoint_folder=None):
        ...
        if passed:
            proposed = Runtime(self.checkpoint, self.snapshot())
            factual, library = proposed.session.learner.legacy.snapshot(), proposed.session.learner.library.record()
            apply(proposed.owner, delta(candidate))
            proposed.owner.inquiry_admitted = [phrase]
            learner = proposed.session.learner
            proposed.reproof = learner.rebind(factual, library)
            proposed.graph.sync(proposed.owner)
            proposed.lessons.append(receipt)
    ...
    def status(self):
        ...
        return {"owner": model_identity(self.owner), "parent_task_owner": self.fit["parent_owner"],
                ...
                "parser": self.graph.parser, "lessons": copy.deepcopy(self.lessons),
...
def main():
    ...
    with lock(directory):
        ...
        if previous is None:
            ...
            runtime = Runtime(previous["checkpoint"], previous, migrate=operation == "migrate")
            ...
            if operation == "ask":
                ...
                graph.run(owner, request.get("ticks", 12))
                response = runtime.status()
            elif operation == "result":
                graph.sync(owner)
                response = copy.deepcopy(graph.jobs[request["id"]])
            elif operation == "pause":
                graph.pause(request["id"])
                response = runtime.status()
            elif operation == "resume":
                graph.resume(owner, request["id"])
                response = runtime.status()
            elif operation == "rebase":
                response = graph.rebase(owner, request["id"])
            elif operation == "clarify":
                graph.clarify(request["id"], request["actions"])
                response = runtime.status()
                ...
                response = {"from": previous["source"], "to": source(), "owner_unchanged": previous["owner"] == model_identity(owner),
                            "reason": "Import/style cleanup and explicit interpreter migration; no training repeated, all derived inquiry states validated",
