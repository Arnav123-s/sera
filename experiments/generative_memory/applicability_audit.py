...
def matrix(kind, t):
    ...
    if kind == 0:
        ...
        a = np.zeros((n, 2, 6))
        # Each basis term stores its x coefficient followed by its y coefficient.
        a[:, 0, 0::2] = np.stack([np.ones(n), sine, cosine], axis=1)
        a[:, 1, 1::2] = np.stack([np.ones(n), sine, cosine], axis=1)
    elif kind == 2:
        a = np.zeros((n, 2, 4))
        a[:, 0, 0::2] = np.stack([np.ones(n), t], axis=1)
        a[:, 1, 1::2] = np.stack([np.ones(n), t], axis=1)
    else:
        a = np.zeros((n, 2, 6))
        a[:, 0, 0], a[:, 1, 1] = 1, 1
        a[:, 0, 2], a[:, 0, 3] = cosine, -sine
...
def audit(run, output):
    ...
    result = {"status": "PASS", "worlds_audited": len(actual), "query_feature_rows": len(actual)*33,
              "auditor_revision": "v3-provenance-and-coefficient-order",
