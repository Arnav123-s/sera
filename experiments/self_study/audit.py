...
def integrate(directory, output):
    ...
    with lock(directory):
        ...
        for domain, learner in session.owner.study_maps.items():
            ...
            x = np.array([[float(v) for v in vector(lesson["input"])] for lesson in lessons])
            y = np.array([[float(v) for v in vector(lesson["output"])] for lesson in lessons])
...
def main():
    ...
    parser.add_argument("mode", choices=("recount", "integrate", "migrate"))
    ...
    if args.mode == "integrate":
        ...
        if not directory.is_relative_to(ROOT / "runs"):
            raise ValueError("Store must remain under runs")
        with lock(directory):
            store = Store(directory)
            previous = store.read()
            session = StudySession(previous["parent"], previous)
            assert session.weights() == previous["weights"]
            assert model_identity(session.owner) == previous["owner"]
            current = store.commit(session.snapshot(), previous)
            answer = session.interpret("set an alarm for nine am")
            write(args.output / "migration.json", {"previous_source": previous["source"],
                  "current_source": current["source"], "owner_unchanged": True,
                  "weights_unchanged": True, "revision_count": store.verify_history(),
                  "corrected_example": answer, "snapshot": session.snapshot()})
            print(json.dumps({"revision": current["revision"], "lineage": answer["training_lineage"]}))
    else:
