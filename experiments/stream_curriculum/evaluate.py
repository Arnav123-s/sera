...
def select(scores):
    ...
    return {**best["checkpoint"], "qualified_annotation_aid": best["intent_accuracy"] >= .70 and best["slot_span_f1"] >= .50,
            "criterion": "development exact frames; then intent accuracy; then path"}


def final_receipt(development, models, supplementary, output, development_path):
    if (development["partition"] != "dev" or development["models"] != models
            or [score["checkpoint"] for score in development["scores"]] != models
            or development["selected"] != select(development["scores"])):
        raise ValueError("Models or selection changed after development")
    # Exclusive cohort receipt is written before any final row is parsed.
    with (OUT / "official-test-access.json").open("x", encoding="utf-8") as receipt:
        json.dump({"models": models, "supplementary": supplementary, "selected": development["selected"],
                   "output": str(output), "development_sha256": sha(development_path),
                   "source": sha(Path(__file__)), "protocol": sha(OUT / "evaluation-protocol.md")}, receipt, indent=2)
...
def main():
    ...
    if args.partition == "test":
        ...
        selected = final_receipt(development, models, supplementary, output, args.development)
    ...
    for index, checkpoint in enumerate([*models, *supplementary]):
        ...
        if checkpoint["path"] in reusable:
            ...
            if any(prior[key] != value for key, value in result.items() if key != "records"):
                raise ValueError("Changed predictions in a reused measurement")
            reused = True
        else:
