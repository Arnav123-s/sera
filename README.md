...
The subsequent [learned-applicability study](research-continuation/15_applicability/report.md) trains and audits a guard on 1,344 distinct worlds. It improves the declared score over fixed controls, but fails the conditional-risk gate on exceptions and changing mechanisms. The complete model, raw cohort, failed checks and exact replay are preserved; it remains an experimental candidate. **174 current tests pass**, and 16,556 prior model/result files remain byte-identical. The [current packet comparison](research-continuation/15_applicability/architecture-audit.md) and [execution checklist](research-continuation/NEXT_ACTIONS.md) identify remaining work. Reliable learned applicability, neural transfer and sustained investigator improvement remain open.
